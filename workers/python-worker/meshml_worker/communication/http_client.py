...
class HTTPClient:
    ...
    def register_worker(
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/sync/workers/register",
                json=payload,
                timeout=self.timeout
            )
            response.raise_for_status()
    ...
    def push_gradients(
        ...
        try:
            response = self.session.post(
                f"{self.base_url}/gradients/submit",
                json=payload,
                timeout=self.timeout
            )
            response.raise_for_status()
        ...
        try:
            ...
            if "parameters" in result:
                ...
                for name, tensor_data in result["parameters"].items():
                    if isinstance(tensor_data, dict) and "shape" in tensor_data and "data" in tensor_data:
    ...
    def get_model_version(self, model_id: str) -> int:
        ...
        try:
            response = self.session.get(
                f"{self.base_url}/parameters/{model_id}/version",
                timeout=self.timeout
            )
            response.raise_for_status()
            result = response.json()
    ...
    def unregister_worker(self, worker_id: str) -> bool:
        ...
        try:
            response = self.session.delete(
                f"{self.base_url}/sync/workers/{worker_id}",
                timeout=self.timeout
            )
            response.raise_for_status()
