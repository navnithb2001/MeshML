...
class TaskAssignmentService:
    ...
    async def _select_worker(
        ...
        workers = self.worker_discovery.get_available_workers(
            group_id=constraints.require_group,
            min_gpu_count=constraints.min_gpu_count
        )
    ...
    async def assign_batch(
        ...
        logger.info(f"Batch assigning {len(job_ids)} jobs with strategy={strategy.value}, load_balancing={load_balancing.value}")
        ...
        if load_balancing == LoadBalancingPolicy.ROUND_ROBIN:
            ...
            assignments = await self._assign_batch_weighted_round_robin(job_ids, strategy, constraints)
    ...
    async def _assign_batch_weighted_round_robin(
        ...
        worker_weights = [
            (w, w.capabilities.get_compute_score() / total_score)
            for w in workers
        ]
    ...
    async def rebalance_load(
        ...
        for overloaded_worker in overloaded:
            ...
            # Get jobs to move
            jobs_to_move = int((overloaded_worker.utilization - threshold) * overloaded_worker.total_capacity)
            ...
            for job_id in jobs:
                ...
                if success:
                    ...
                    target_worker.utilization = target_worker.assigned_jobs / target_worker.total_capacity
