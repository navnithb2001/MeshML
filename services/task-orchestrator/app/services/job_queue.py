...
class JobInfo:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "JobInfo":
        ...
        if "priority" in data and isinstance(data["priority"], (int, str)):
            data["priority"] = JobPriority(data["priority"]) if isinstance(data["priority"], str) else JobPriority(data["priority"])
        return cls(**data)
...
class JobQueue:
    ...
    def update_job_status(
        ...
        if not self._is_valid_transition(old_status, new_status):
            logger.warning(f"Invalid transition for job {job_id}: {old_status.value} → {new_status.value}")
            return False
        ...
        if new_status in {JobStatus.COMPLETED, JobStatus.FAILED, JobStatus.CANCELLED, JobStatus.TIMEOUT}:
    ...
    def release_job_from_worker(
        ...
        try:
            ...
            if job_info.can_retry():
                ...
                logger.info(f"Job {job_id} released and re-queued (retry {job_info.retry_count}/{job_info.max_retries})")
            else:
                # Mark as permanently failed
                self.update_job_status(job_id, JobStatus.FAILED, error_message="Max retries exceeded")
    ...
    def _matches_requirements(self, job_info: JobInfo, requirements: JobRequirements) -> bool:
        ...
        return (
            requirements.min_gpu_count >= job_req.min_gpu_count and
            requirements.min_gpu_memory_gb >= job_req.min_gpu_memory_gb and
            requirements.min_cpu_count >= job_req.min_cpu_count and
            requirements.min_ram_gb >= job_req.min_ram_gb and
            (not job_req.requires_cuda or requirements.requires_cuda) and
            (not job_req.requires_mps or requirements.requires_mps)
        )
    ...
    def mark_validation_complete(
        ...
        try:
            ...
            if model_validation_passed and dataset_validation_passed:
                ...
                error_msg = f"Validation failed: {', '.join(validation_errors or ['Unknown error'])}"
    ...
    def cancel_job(self, job_id: str, reason: str = "user_requested") -> bool:
        ...
        try:
            ...
            # Update status
            self.update_job_status(job_id, JobStatus.CANCELLED, error_message=f"Cancelled: {reason}")
