...
class WorkerDiscoveryService:
    ...
    def register_worker(
        ...
        if group_id and group_id not in self.pools:
            self.create_pool(
                group_id=group_id,
                name=f"Pool for {group_id}",
                description="Auto-created pool"
            )
        ...
        if group_id:
            ...
            if pool.is_at_capacity():
                raise ValueError(f"Pool {group_id} is at maximum capacity ({pool.max_workers} workers)")
    ...
    def match_worker_to_job(
        ...
        for worker in available_workers:
            ...
            if (caps.gpu_count >= job_req.min_gpu_count and
                caps.gpu_memory_gb >= job_req.min_gpu_memory_gb and
                caps.cpu_count >= job_req.min_cpu_count and
                caps.ram_gb >= job_req.min_ram_gb and
                (not job_req.requires_cuda or caps.supports_cuda) and
                (not job_req.requires_mps or caps.supports_mps)):
        ...
        logger.info(f"Matched job {job_id} to worker {best_worker.worker_id} "
    ...
    def assign_job_to_worker(
        ...
        worker_success = self.worker_registry.assign_job(
            worker_id=worker_id,
            job_id=job_id,
            shard_id=shard_ids[0] if shard_ids else None
        )
    ...
    def check_scaling_needs(self) -> Dict[str, str]:
        ...
        for group_id, pool in self.pools.items():
            ...
            if active_count < pool.min_workers:
                ...
                logger.info(f"Pool {group_id} needs scaling up: {active_count}/{pool.min_workers} workers")
