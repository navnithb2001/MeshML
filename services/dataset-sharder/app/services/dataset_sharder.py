...
from app.services.dataset_loader import DatasetLoader, DatasetMetadata, DataSample
...
class DatasetSharder:
    ...
    def create_shards(self) -> List[ShardMetadata]:
        ...
        logger.info(f"Creating {self.config.num_shards} shards using {self.config.strategy} strategy")
        logger.info(f"Dataset: {self.metadata.total_samples} samples, {self.metadata.num_classes} classes")
...
def analyze_distribution_quality(shards: List[ShardMetadata]) -> Dict[str, Any]:
    ...
    return {
        ...
        "class_variance": {
            "per_class": class_variances,
            "average": avg_class_variance
        },
