...
class ImageFolderLoader(DatasetLoader):
    ...
    def _load_metadata_local(self) -> DatasetMetadata:
        ...
        logger.info(f"Loaded ImageFolder metadata: {total_samples} samples, {len(class_names)} classes")
    ...
    def _load_metadata_gcs(self) -> DatasetMetadata:
        ...
        gcs_path = self.dataset_path.replace("gs://", "")
        ...
        for blob in blobs:
            ...
            if len(parts) >= 2:  # class_name/image.jpg
                ...
                if Path(filename).suffix.lower() in image_extensions:
                    ...
                    class_files[class_name].append((blob.name, blob.size))
        ...
        for class_name, files in class_files.items():
            ...
            for blob_name, _ in files:
                self.samples.append((f"gs://{bucket_name}/{blob_name}", self.class_to_idx[class_name]))
        ...
        logger.info(f"Loaded GCS ImageFolder metadata: {total_samples} samples, {len(class_names)} classes")
        return self.metadata
...
class COCOLoader(DatasetLoader):
    ...
    def load_metadata(self) -> DatasetMetadata:
        ...
        if self._is_gcs:
            ...
            gcs_path = annotations_path.replace("gs://", "")
            bucket_name = gcs_path.split("/")[0]
            blob_name = "/".join(gcs_path.split("/")[1:])
            
            bucket = storage_client.bucket
            blob = bucket.blob(blob_name)
            annotations_json = blob.download_as_text()
    ...
    def _load_local_coco_image(self, filepath: str, img_info: Dict[str, Any], idx: int) -> DataSample:
...
class CSVLoader(DatasetLoader):
    ...
    def load_metadata(self) -> DatasetMetadata:
        ...
        logger.info(f"Loaded CSV metadata: {len(self.data)} samples, {len(unique_labels)} classes, {len(self.feature_columns)} features")
        return self.metadata
    ...
    def get_sample(self, index: int) -> DataSample:
        ...
        return DataSample(
            ...
            sample_id=f"csv_{index}"
        )


def create_loader(dataset_path: str, format: Optional[DatasetFormat] = None, **kwargs) -> DatasetLoader:
    """
