...
async def get_worker_assignment(
    ...
    if not assignment:
        raise HTTPException(
            status_code=404,
            detail=f"No assignment found for worker {worker_id}"
        )
...
async def get_batch_assignment(
    ...
    if not assignment:
        raise HTTPException(
            status_code=404,
            detail=f"No assignment found for batch {batch_id}"
        )
...
async def download_batch(
    ...
    if not assignment:
        raise HTTPException(
            status_code=404,
            detail=f"Batch {batch_id} not assigned"
        )
    
    if assignment.worker_id != worker_id:
        raise HTTPException(
            status_code=403,
            detail=f"Batch {batch_id} not assigned to worker {worker_id}"
        )
    ...
    try:
        ...
        # Mark as failed
        distributor.mark_download_failed(
            worker_id,
            batch_id,
            "Batch file not found in storage"
        )
...
async def reassign_batch(
    ...
    new_assignment = distributor.reassign_failed_batch(
        batch_id=batch_id,
        new_worker_id=request.new_worker_id
    )
    
    if not new_assignment:
        raise HTTPException(
            status_code=400,
            detail="Failed to reassign batch (may have exceeded max retries)"
        )
