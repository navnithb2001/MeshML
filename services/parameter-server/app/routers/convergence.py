...
class ConvergenceConfigRequest(BaseModel):
    ...
    loss_threshold: Optional[float] = Field(default=None, description="Loss threshold for convergence")
    ...
    target_metrics: Dict[str, Tuple[float, str]] = Field(
        ...
        description="Target metrics: {name: (value, direction)}"
    )
    ...
    gradient_norm_threshold: Optional[float] = Field(
        default=None,
        description="Gradient norm threshold"
    )
...
async def update_metrics(
    ...
    config: Optional[ConvergenceConfigRequest] = None
...
async def get_convergence_summary(
    ...
    try:
        ...
        if "status" in summary:
            ...
            raise HTTPException(
                status_code=404,
                detail=f"No convergence data for {model_id}"
            )
...
async def get_metrics_history(
    ...
    """
    try:
        history = convergence_service.get_metrics_history(
            model_id=model_id,
            limit=limit
        )
