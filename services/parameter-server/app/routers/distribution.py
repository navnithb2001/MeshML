...
class PullParametersRequest(BaseModel):
    ...
    requested_version: Optional[int] = Field(default=None, description="Requested version (None = latest)")
    ...
    format_type: ParameterFormat = Field(default=ParameterFormat.PYTORCH, description="Parameter format")
    parameter_names: Optional[List[str]] = Field(default=None, description="Specific parameters (None = all)")
...
class BroadcastParametersRequest(BaseModel):
    ...
    version_id: Optional[int] = Field(default=None, description="Version to broadcast (None = latest)")
    compression: CompressionType = Field(default=CompressionType.NONE, description="Compression type")
    format_type: ParameterFormat = Field(default=ParameterFormat.PYTORCH, description="Parameter format")
...
async def pull_parameters(
    request: PullParametersRequest
) -> ParameterPackageResponse:
    """
...
async def broadcast_parameters(
    request: BroadcastParametersRequest
) -> Dict[str, Any]:
    """
...
async def subscribe_to_updates(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": "Subscribed to parameter updates" if newly_subscribed else "Already subscribed",
            "model_id": request.model_id,
            "worker_id": request.worker_id,
