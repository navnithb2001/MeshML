...
class ModelInitializerService:
    ...
    async def initialize_model(
        ...
        try:
            ...
            local_model_path = await self._download_model_from_gcs(
                config.gcs_model_path
            )
            
            # 2. Import and validate model module
            create_model_fn, metadata = self._import_and_validate_model(
                local_model_path
            )
            ...
            if not isinstance(model, nn.Module):
                raise ValueError(
                    f"create_model() must return nn.Module, got {type(model)}"
                )
    ...
    async def _download_model_from_gcs(self, gcs_path: str) -> str:
        ...
        local_path = os.path.join(
            self.temp_dir,
            f"model_{hashlib.md5(gcs_path.encode()).hexdigest()}.py"
        )
    ...
    def _import_and_validate_model(
        self,
        model_path: str
    ) -> Tuple[Callable, ModelMetadata]:
        """
        ...
        if metadata_dict["framework"].lower() != "pytorch":
            raise ValueError(
                f"Only PyTorch models supported, got: {metadata_dict['framework']}"
            )
        ...
        metadata = ModelMetadata(
            ...
            custom_fields={
                ...
                if k not in [
                    "name", "version", "framework", "input_shape",
                    "output_shape", "num_parameters", "description",
                    "author", "tags"
                ]
            }
        )
    ...
    def _initialize_weights(
        ...
        if strategy == InitializationStrategy.PRETRAINED:
            ...
            if not config.pretrained_weights_path:
                raise ValueError(
                    "pretrained_weights_path required for PRETRAINED strategy"
                )
    ...
    async def _download_weights_from_gcs(self, gcs_path: str) -> str:
        ...
        local_path = os.path.join(
            self.temp_dir,
            f"weights_{hashlib.md5(gcs_path.encode()).hexdigest()}.pt"
        )
    ...
    def get_model_info(self, model_id: str) -> Optional[Dict[str, Any]]:
        ...
        return {
            ...
            "metadata": {
                ...
                "version": initialized_model.metadata.version if initialized_model.metadata else None,
                "framework": initialized_model.metadata.framework if initialized_model.metadata else None,
                "description": initialized_model.metadata.description if initialized_model.metadata else None,
            } if initialized_model.metadata else None,
    ...
    async def reinitialize_model(
        self,
        model_id: str,
        new_strategy: Optional[InitializationStrategy] = None
    ) -> InitializedModel:
        """
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_parameters = sum(
            m.num_parameters for m in self.models.values()
            if m.num_parameters
        )
