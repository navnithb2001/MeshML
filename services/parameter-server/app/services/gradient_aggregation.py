...
class GradientAggregationService:
    ...
    def aggregate_gradients(
        ...
        if not valid_updates:
            logger.warning(
                f"All gradients too stale for {model_id} "
        ...
        if config.strategy == AggregationStrategy.FEDAVG:
            aggregated = self._federated_averaging(
                valid_updates,
                staleness_weights
            )
        elif config.strategy == AggregationStrategy.SIMPLE_AVERAGE:
            aggregated = self._simple_average(
                valid_updates,
                staleness_weights
            )
        ...
        for update in updates:
            ...
            for name, grad in update.gradients.items():
                if config.clipping_strategy == ClippingStrategy.VALUE:
                    # Clip by value
                    clipped_grads[name] = torch.clamp(
                        grad,
                        -config.clip_value,
                        config.clip_value
                    )
    ...
    def _federated_averaging(
        ...
        total_weighted_samples = sum(
            u.num_samples * staleness_weights[u.worker_id]
            for u in updates
        )
        ...
        for name in param_names:
            ...
            for update in updates:
                ...
                weight = (update.num_samples * staleness_weights[update.worker_id]) / total_weighted_samples
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        pending_gradients = sum(
            len(grads) for grads in self.gradient_buffer.values()
        )
        
        models_with_pending = len([
            model_id for model_id, grads in self.gradient_buffer.items()
            if grads
        ])
