...
class ConvergenceDetectionService:
    ...
    def __init__(
        self,
        default_config: Optional[ConvergenceConfig] = None
    ):
        """
            ...
            if metric_name in current_metrics.metrics:
                ...
                if direction == MetricDirection.MAXIMIZE:
                    if metric_value >= target_value:
                        ...
                        converged = True
                        message = f"Target {metric_name} reached ({metric_value:.4f} >= {target_value})"
    ...
    def get_convergence_summary(self, model_id: str) -> Dict[str, Any]:
        ...
        loss_history = [m.loss for m in history[-10:]] if len(history) >= 10 else [m.loss for m in history]
