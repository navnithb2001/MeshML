...
class SynchronizationService:
    ...
    async def _handle_asynchronous(
        ...
        if len(pending) >= config.async_batch_size:
            # Aggregate now
            logger.info(
                f"Async batch size reached ({len(pending)} gradients), aggregating"
            )
    ...
    async def _complete_sync_round(
        ...
        # Aggregate gradients
        result = self._aggregate_gradients(
            model_id,
            current_round.target_version,
            config
        )
    ...
    async def _schedule_async_aggregation(
        self,
        model_id: str,
        config: SyncConfig
    ) -> None:
        ...
        async def aggregate_after_timeout():
            ...
            if pending:
                logger.info(
                    f"Async timeout reached, aggregating {len(pending)} gradients"
                )
                result = self._aggregate_gradients(
                    model_id,
    ...
    def add_aggregation_callback(
        self,
        callback: Callable[[AggregatedGradient], None]
    ) -> None:
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        timed_out_workers = len([w for w in self.workers.values() if w.state == WorkerState.TIMED_OUT])
