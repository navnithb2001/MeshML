...
from ..schemas import ModelResponse, ModelListResponse, ModelSearchQuery, ModelUsageStats
import logging
...
async def search_models(
    ...
    logger.info(
        f"Search returned {len(models)} models (page {page}, total {total})"
    )
...
async def list_architecture_types(
    db: AsyncSession = Depends(get_db_session)
):
...
async def get_model_usage(
    ...
    usage_result = await db.execute(
        select(ModelUsage).where(ModelUsage.model_id == model_id)
    )
