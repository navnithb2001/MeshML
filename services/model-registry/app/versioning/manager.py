...
class VersionManager:
    ...
    async def create_version(
        ...
        result = await self.db.execute(
            select(Model).where(Model.id == parent_model_id)
        )
        ...
        if parent.state != ModelState.READY:
            raise ValueError(
                f"Parent model must be in READY state, currently {parent.state}"
            )
        ...
        if existing.scalar_one_or_none():
            raise ValueError(
                f"Version {version} already exists for model {parent_model_id}"
            )
        ...
        logger.info(
            f"Created version {version} of model {parent_model_id} -> {new_model.id}"
        )
    ...
    def increment_version(
        self, 
        current_version: str,
        part: str = "patch"  # major, minor, or patch
    ) -> str:
        """
    ...
    async def suggest_next_version(self, parent_model_id: int) -> str:
        ...
        if not versions:
            ...
            result = await self.db.execute(
                select(Model.version).where(Model.id == parent_model_id)
            )
        ...
        latest_version = max(
            versions,
            key=lambda m: self.parse_version(m.version)
        ).version
