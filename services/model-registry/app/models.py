...
class Model(Base):
    ...
    # State & Lifecycle
    state = Column(String(50), default="uploading", index=True)  # Use String instead of Enum for compatibility
...
class ModelUsage(Base):
    ...
    user_id = Column(UUID(as_uuid=True), nullable=True, index=True)  # No FK to avoid circular dependency
